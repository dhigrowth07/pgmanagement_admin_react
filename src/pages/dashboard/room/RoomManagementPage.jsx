...
import {
  fetchRoomsData, addBlock, updateBlock, deleteBlock,
  addRoom, updateRoom, deleteRoom,
  addTariff, updateTariff, deleteTariff,
  addRoomPreset, updateRoomPreset, deleteRoomPreset,
...
const RoomManagementPage = () => {
  ...
  const filteredBlocks = useMemo(() => {
    ...
    if (filters.block !== 'all') blockData = blockData.filter(b => b.block_id === filters.block);
  ...
  const presetColumns = [
    ...
    {
      title: 'Tariff', dataIndex: 'tariff_name', key: 'tariff_name',
  ...
  return (
    ...
    >
      <Row justify="space-between" align="middle" gutter={[0, 0]} style={{ marginBottom: 16 }}>
        <Col xs={24} md={8}>
          <Title level={4} className="pt-0 md:text-left text-center">Rooms Management</Title>
        </Col>
        <Col xs={24} md={16} className="md:text-right text-center">
          <Space wrap={isMobile}>
            ...
            <Button onClick={() => openModal('addBlock')} icon={<PlusOutlined />}>Add Block</Button>
            <Button onClick={() => openModal('addRoom')} type="primary" icon={<PlusOutlined />}>Add Room</Button>
          </Space>
        </Col>
      </Row>
      ...
      <GoogleReviewModal
        ...
        block={reviewModal.block}
        onSubmit={handleUpdateReviewLink}
        loading={status === 'loading'}
      />
